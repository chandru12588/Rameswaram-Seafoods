...
export default function AdminOrderView(){
  ...
  if(!order) return <p className="text-center mt-28 text-lg">Loading order details...</p>
  ...
  return (
    <div className="pt-28 p-6 max-w-3xl mx-auto">
      ...
      <ul className="bg-gray-100 p-4 rounded">
        {order.items.map((i,j)=>(
          <li key={j} className="py-1">• {i.name} × {i.quantity} — ₹{i.price}</li>
        ))}
      </ul>

      <a
        target="_blank"
        href={`https://wa.me/919655244550?text=📦 *New Order*%0A%0A🧾 *Order ID:* ${order._id}%0A👤 *${order.customerName}*%0A📞 ${order.customerMobile}%0A🏠 ${order.customerAddress}%0A%0A${order.items.map(i=>`• ${i.name} × ${i.quantity}`).join("%0A")}%0A%0A💰 *Total:* ₹${order.totalAmount}`}
